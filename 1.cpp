...
void pushNumber(Stack* stack, int number) {
    ...
    stack->head = newNode; // обновляется вершина стека
}
    ...
    for (size_t i = 0; i < expression.length(); i++) {
        ...
        // Если оператор
            ...
            while (operators.head != nullptr && precedence(operators.head->op) >= precedence(token)) { 
                result += operators.head->op; // все операторы с более высоким или равным приоритетом извлекаются из стека и добавляются в result
                result += ' ';
                pop(&operators);
            }
        ...
        // Если открывающая скобка
            ...
            pushOperator(&operators, token); // помещается в стек
        }
